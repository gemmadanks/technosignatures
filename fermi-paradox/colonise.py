...
def generate_galaxy(num_stars, radius, thickness, seed=1):
    """Generate DataFrame containing Cartesian coordinates of stars.

    Stars will be arranged in a disc with a normal distribution
    (peaking in galactic centre and middle of disc height).
    States of each star are initialised:
    state = "unknown"; colony age = 0; 
    time until colonisation and year of colonisation = np.NaN.

    Args:
        num_stars (int): number of stars in galaxy
        radius: radius of galaxy
        thickness: thickness of galaxy
    Returns:
        galaxy (pd.DataFrame): Cartesian coordinates and states of stars.
    """
...
def normal_distribution_on_circle(num_stars, radius, seed):
    """Generate positions of stars in a normal distribution on a circle.
    
    Args:
        num_stars (int): number of stars
        radius (float): radius of circle
        seed (float): seed for random number generator
    Returns:
         x_coords (list): list of x Cartesian coordinates
         y_coords (list): list of y Cartesian coordinates
    """
...
def evolve_eti(seed, star_map):
    """Randomly select star for emergence of first ETI civilisation.

    Args:
        seed (float): seed for random number generator
        star_map (pd.DataFrame): Cartesian coordinates and states of stars
    Returns:
        star_map (pd.DataFrame): Cartesian coordinates and states of stars
                                 updated with 'eti' state for selected star
        distance_from_galactic_centre (float): Distance from centre of galaxy of ETI
    """
...
def plot_galaxy(star_map):
    """Plot galaxy of stars in 3D.

    Cartesian coordinates are used to plot positions of stars.
    Colours represent states.
...
def colonise_galaxy(star_map, search_radius, travel_speed,
    ...
    """Simulate galactic colonisation by ETI.

    Args:
        star_map (pd.DataFrame): Cartesian coordinates and states of stars
        search_radius (float): Maximum radius to search for nearby stars (in light years)
        travel_speed (float): Fraction of speed of light for interstellar travel
        max_num_targets (int): Number of closest stars to target (default no maximum)
        mission_success_rate (float): Probability of success of each mission (number between 0.0 and 1.0)
        planning_time (int): How many years to wait between colonisation and next mission (default 0)
        longevity (int): Number of years each colony will survive
    Returns:
        star_map (pd.DataFrame): Cartesian coordinates and final states of stars
    """
    print("ETI is ready to colonise the galaxy...")
    missions = plan_new_missions(star_map, search_radius, travel_speed, max_num_targets, planning_time)
    star_map = update_starmap(missions, star_map)
    print(f"{len(missions)} new missions launched from newly emerged ETI!\n")
    year = 1
    colonising = True 
    while colonising:
        star_map = progress_colonies(star_map, longevity)
        star_map = progress_missions(star_map, year, mission_success_rate)    
...
def plan_new_missions(star_map, search_radius, travel_speed, max_num_targets, planning_time=0):
    """Plan new missions to nearby stars for each inhabited star.
    ...
    Args:
        ...
        planning_time (int): How many years to wait between colonisation and next mission (default 0)
    Returns:
        new_missions (dict): Dictionary of star ids and number of years to colonisation
    """
...
def update_starmap(new_missions, star_map):
    """Update states of stars for new missions planned.

    Args:
        star_map (pd.DataFrame): Cartesian coordinates and states of stars
        new_missions (dict): Dictionary of star ids and number of years to colonisation
    Returns:
        star_map (pd.DataFrame): Cartesian coordinates and updated states of stars
    """
...
def progress_colonies(star_map, longevity=None):
    """Update states of colonised stars.

    Increase age of each colony until it reaches longevity. 

    Args:
        star_map (pd.DataFrame): Cartesian coordinates and states of stars
        longevity (int): Number of years a colony can survive (default is infinite)
    Returns:
        star_map (pd.DataFrame): Cartesian coordinates and updated states of stars
    """
...
def progress_missions(star_map, year, mission_success_rate=1.0):
    """Update states of targeted stars.

    Decrease time to colonisation until star is reached by ETI. Once reached star is colonised.

    Args:
        star_map (pd.DataFrame): Cartesian coordinates and states of stars
        year (int): Years since colonisation began
        mission_success_rate (float): Probability of success of each mission (number between 0.0 and 1.0)
    Returns:
        star_map (pd.DataFrame): Cartesian coordinates and updated states of stars
    """
...
def find_neighbours(star_dict, radius, star_map):
    ...
    Args:
        star_dict (dict): dictionary of x, y, z coordinates for host star 
        radius (float): radius to search
        star_map (pd.DataFrame): Cartesian coordinates and states of stars
    Returns:
        neighbours (pd.DataFrame): Cartesian coordinates and states of neighbours
    """
...
def find_targets(neighbours, max_num_targets=None):
    """Find closest nearby stars that are not already inhabited.

    Stars with existing or extinct colonies and those already targetted by another colony
    are not targetted again. 

    Args:
        neighbours (dict): dictionary of x, y, z coordinates for nearby stars 
        max_num_targets (int): Number of closest stars to target (default no maximum)
    Returns:
        targets (pd.DataFrame): Cartesian coordinates and states of targets
    """
...
def time_to_colonisation(targets, travel_speed):
    """Compute travel time to target stars.

    Args:
        targets (pd.DataFrame): Cartesian coordinates and states of targets
        travel_speed (float): Fraction of speed of light for interstellar travel
    Returns:
        (dict): Dictionary of target star ids and travel time
    """
...
def colonise_stars(star_map, num_stars_reached, mission_success_rate, year):
    """Colonises stars where missions are successful.

    Targets of failed missions can be targetted by another colony.

    Args:
        star_map (pd.DataFrame): Cartesian coordinates and states of stars
        num_stars_reached (int):
        mission_success_rate (float): Probability of success of each mission (number between 0.0 and 1.0)
        year (int): Years since colonisation began
    Returns:
        star_map (pd.DataFrame): Cartesian coordinates and updated states of stars
    """
    missions_succeeded = np.random.default_rng(seed=None).binomial(1,
                                                                   mission_success_rate,
                                                                   num_stars_reached).astype(bool)
...
def report(star_map):
    """Generate report and plots for status of galaxy.
